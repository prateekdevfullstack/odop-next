...
export default function DistrictSupervisorDistrictPage() {
  ...
  const loadSummary = useCallback(async (districtId?: string) => {
    ...
    try {
      ...
      setSummary(response.data.data ?? EMPTY_SUMMARY);
    } catch (err) {
  ...
  return (
    <div className="ds-dashboard">
      <section className="ds-filter-bar" aria-label="District filters">
        <span id="ds-full-district-filter-label">Filter by District</span>
        <div className="ds-select" ref={districtDropdownRef}>
          <button
            type="button"
            className="ds-select__button"
            aria-labelledby="ds-full-district-filter-label"
      ...
      <section className="ds-overview-panel" aria-labelledby="ds-all-district-title">
        <h2 id="ds-all-district-title">District Overview</h2>
        <div className="ds-dashboard-grid" aria-label="All district profile summaries">
          {isLoading ? (
            <p className="ds-dashboard-status">Loading...</p>
          ) : summary.data.length > 0 ? (
            summary.data.map((district, index) => (
              <article

...
import { parseExporterFormUpdate } from "@/lib/validation/district-admin-exporter.schema";
import {
  buildExporterFormData,
  getDistrictExporter,
  exporterToFormValues,
  updateDistrictExporter,
...
export default function EditDistrictExporterPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setValues(
        bootstrapDistrictFormValues(exporterToFormValues(res.data, districtName), districtName)
      );
      setProductCategories(pc);
      setCategories(cat);
    } catch {
      toast.error("Failed to load exporter.");
      router.push(
        districtAdminExportersPath(
          districtSlug,
          productCategoryQueryFromId(urlCategoryId)
  ...
  if (loading || !values) {
    return <LoadingState label="Loading exporter…" />;
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const parsed = parseExporterFormUpdate(values);
    if (!parsed.success) {
    ...
    try {
      ...
      await updateDistrictExporter(id, fd);
      toast.success("Exporter updated successfully.");
      router.push(districtAdminExporterDetailPath(districtSlug, id, categoryQuery));
    } catch (err) {

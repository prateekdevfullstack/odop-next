...
import { addUserContactEnquiryComment } from "@/services/user-contact-enquiry.service";

const MAX_REMARKS_LENGTH = 5000;

type ContactEnquiryCommentFormProps = {
  enquiryId: number;
  disabled?: boolean;
  onSuccess: () => void;
};

export function ContactEnquiryCommentForm({
  enquiryId,
  disabled = false,
  onSuccess,
}: ContactEnquiryCommentFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      await addUserContactEnquiryComment(enquiryId, trimmed);
  ...
  if (disabled) {
    return (
      <div className="grievance-comment-readonly panel">
        <p>This enquiry is closed for further comments.</p>
      </div>
    );
  }
  ...
  return (
    <form className="grievance-comment-form panel" onSubmit={(e) => void handleSubmit(e)}>
      ...
      <p className="panel-meta">
        Share any extra details that may help resolve your enquiry.
      </p>
      <div className="grievance-form-field full-span">
        <label htmlFor="contact-enquiry-comment">Your message</label>
        <textarea
          id="contact-enquiry-comment"
          rows={4}

...
const SEGS = [
  {
    id: "heritage",
    lines: ["Heritage &", "Identity"],
    linesHi: ["विरासत एवं", "पहचान"],
    s: 270, e: 342, mid: 306,
    color: "#00AFA5",
  ...
  {
    id: "artisan",
    lines: ["Artisan", "Empowerment"],
    linesHi: ["शिल्पकार", "सशक्तिकरण"],
    s: 342, e: 54, mid: 18,
    color: "#AC25B0",
  ...
  {
    id: "market",
    lines: ["Market &", "Export Promotion"],
    linesHi: ["बाजार एवं", "निर्यात प्रोत्साहन"],
    s: 54, e: 126, mid: 90,
    color: "#2764B4",
  ...
  {
    id: "economic",
    lines: ["Economic", "Growth"],
    linesHi: ["आर्थिक", "विकास"],
    s: 126, e: 198, mid: 162,
    color: "#E2A900",
  ...
  {
    id: "enterprise",
    lines: ["Enterprise &", "Infrastructure"],
    linesHi: ["उद्यम एवं", "अवसंरचना"],
    s: 198, e: 270, mid: 234,
    color: "#148EFF",
...
const LINE_H = 17;
// Label sits radially outside badge outer edge
...
export default function CharkhaWheel({ activeId, onSelect, rotation }: Props) {
  ...
  return (
    ...
    >
      <svg
        role="img"
        aria-label="ODOP Charkha Wheel — five programme pillars"
        viewBox={`-80 -80 ${VB + 160} ${VB + 160}`}
        width="100%"
        xmlns="http://www.w3.org/2000/svg"
      >
        <title>ODOP Programme Pillars</title>

        <defs>
          {SEGS.map((seg) => (
            <clipPath key={seg.id} id={`cp-${seg.id}`}>
          ...
          <filter id="f-center" x="-30%" y="-30%" width="160%" height="160%">
            <feDropShadow dx="0" dy="4" stdDeviation="10" floodOpacity="0.12" />
          </filter>
        </defs>

        {/* ── Rotating group: segments + labels ── */}
        <g ref={groupRef}>
          {SEGS.map((seg) => {
        ...
        <circle cx={CX} cy={CY} r={R_IN - 5} fill="white" filter="url(#f-center)" />
        <image
        ...
        {SEGS.map((seg, i) => {
          const isActive = seg.id === activeId;
          // Initial transform set imperatively; element renders at (0,0) until effect fires
